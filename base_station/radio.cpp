...
// Rate-limit identical log lines. See rocket_avionics/radio.cpp for design notes.
...
static bool logRate(uint8_t slot, uint32_t intervalMs, uint32_t* suppressedOut = nullptr) {
  ...
  if (slot >= LR_NUM_SLOTS) return true;
  uint32_t now = millis();
...
enum : uint8_t {
  ...
  LRSLOT_TX_ATTEMPT       = 5,
  LRSLOT_TX_FAIL          = 6,
  LRSLOT_BUSY_PRE_MOD     = 7,
  LRSLOT_FREQ_FAIL        = 8,
  LRSLOT_SCAN_SEARCHING   = 9,
};
...
bool bsRadioInit() {
  ...
  // Each of these can silently fail if BUSY is high — log so init issues are visible.
  st = sx126x_set_reg_mode(&bsRadioCtx, SX126X_REG_MODE_DCDC);
  if (st != SX126X_STATUS_OK) { Serial.print("BS init: set_reg_mode st="); Serial.println(st); }
  st = sx126x_set_pkt_type(&bsRadioCtx, SX126X_PKT_TYPE_LORA);
  if (st != SX126X_STATUS_OK) { Serial.print("BS init: set_pkt_type st="); Serial.println(st); }
  st = sx126x_set_dio2_as_rf_sw_ctrl(&bsRadioCtx, true);
  if (st != SX126X_STATUS_OK) { Serial.print("BS init: set_dio2_as_rf_sw_ctrl st="); Serial.println(st); }

  const uint8_t syncWord[2] = { 0x14, 0x24 };
  st = sx126x_write_register(&bsRadioCtx, 0x0740, syncWord, 2);
  if (st != SX126X_STATUS_OK) { Serial.print("BS init: syncWord write st="); Serial.println(st); }
  ...
  if (st != SX126X_STATUS_OK) { Serial.print("BS init: set_dio_irq_params st="); Serial.println(st); }
  st = sx126x_clear_irq_status(&bsRadioCtx, SX126X_IRQ_ALL);
  if (st != SX126X_STATUS_OK) { Serial.print("BS init: clear_irq_status st="); Serial.println(st); }

  st = sx126x_cfg_rx_boosted(&bsRadioCtx, LORA_RX_BOOSTED);
  if (st != SX126X_STATUS_OK) { Serial.print("BS init: cfg_rx_boosted st="); Serial.println(st); }
  ...
  Serial.print("BS HAL drops during init: write="); Serial.print(totalBusyWriteDrops);
...
void bsRadioApplyConfig_BLOCKING() {
  ...
  st = sx126x_set_lora_pkt_params(&bsRadioCtx, &pp);
  if (st != SX126X_STATUS_OK) { Serial.print("BS ApplyConfig: set_lora_pkt_params st="); Serial.println(st); }
  DO_NOT_CALL_WHILE_ARMED_radioWaitBusy_WARNING_LONG_BLOCKING(&bsRadioCtx);

  uint32_t freqHz = (uint32_t)(activeFreqMHz * 1e6f + 0.5f);
  st = sx126x_set_rf_freq(&bsRadioCtx, freqHz);
  if (st != SX126X_STATUS_OK) { Serial.print("BS ApplyConfig: set_rf_freq st="); Serial.println(st); }
  DO_NOT_CALL_WHILE_ARMED_radioWaitBusy_WARNING_LONG_BLOCKING(&bsRadioCtx);
  ...
  if (st != SX126X_STATUS_OK) { Serial.print("BS ApplyConfig: set_pa_cfg st="); Serial.println(st); }
  DO_NOT_CALL_WHILE_ARMED_radioWaitBusy_WARNING_LONG_BLOCKING(&bsRadioCtx);

  st = sx126x_set_tx_params(&bsRadioCtx, activePower, SX126X_RAMP_200_US);
  if (st != SX126X_STATUS_OK) { Serial.print("BS ApplyConfig: set_tx_params st="); Serial.println(st); }
  DO_NOT_CALL_WHILE_ARMED_radioWaitBusy_WARNING_LONG_BLOCKING(&bsRadioCtx);
...
void bsRadioStartRxTimeout(uint32_t timeoutRtcSteps,
  ...
  uint32_t supp;
  if (LOG_BS_RX_ATTEMPT && logRate(LRSLOT_RX_ATTEMPT, 1000, &supp)) {
  ...
  if (stMod != SX126X_STATUS_OK) {
    if (logRate(LRSLOT_RX_MOD_FAIL, 1000, &supp)) {
      logSpiFail("BS RX: set_lora_mod_params FAIL", stMod, busyBefore, waited, busyAfter,
  ...
  if (st == SX126X_STATUS_OK) {
    ...
    if (LOG_BS_RX_ATTEMPT && logRate(LRSLOT_RX_SETRX_FAIL, 1000)) {
...
void bsHandleRadio() {
  ...
  if (bsScanState == SCAN_SEARCHING) {
    ...
    if (bsRadioState == BS_RADIO_STANDBY) {
      ...
      if (LOG_BS_SCAN_RX_START) {
        uint32_t scanSupp;
        if (logRate(LRSLOT_SCAN_SEARCHING, 1000, &scanSupp)) {
          Serial.print("BS SCAN: searching on ch="); Serial.print(scanCh);
          Serial.print(" radioState="); Serial.print((int)bsRadioState);
          Serial.print(" supp="); Serial.println(scanSupp);
        }
      }
    }
    return;
